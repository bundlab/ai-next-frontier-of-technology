...
class TaskExecutionPlan(BaseModel):
    ...
    confidence_score: float = Field(..., ge=0.0, le=1.0, description="Model confidence rating")
    requires_human_approval: bool = Field(default=False, description="Manual gate trigger")
    steps: List[TaskStep] = Field(..., min_items=1, description="List of execution steps")
...
class SimulatedLLMProvider:
    ...
    def generate_completion(self, prompt: str) -> str:
        self._call_count += 1
        logger.info(f"LLM API Invoked (Call #{self._call_count}) with prompt: '{prompt}'")

        # Simulate initial failures to demonstrate schema validation & recovery
        if self._call_count == 1:
            logger.warning("Simulating LLM output with invalid types (string instead of float for confidence)...")
            return json.dumps({
                "task_name": "Database Schema Migration",
                "confidence_score": "INVALID_HIGH_CONFIDENCE",  # Causes Pydantic ValidationError
                "requires_human_approval": False,
                "steps": [
                    {"step_id": 1, "action": "Run alembic upgrade head", "target_resource": "PostgreSQL"}
                ]
            })
        ...
        return json.dumps({
            ...
            "requires_human_approval": False,
            "steps": [
                {"step_id": 1, "action": "Create read-only replica snapshot", "target_resource": "RDS Cluster"},
                {"step_id": 2, "action": "Execute non-blocking schema migration", "target_resource": "PostgreSQL Primary"},
                {"step_id": 3, "action": "Validate table index integrity", "target_resource": "PostgreSQL Primary"}
            ]
        })
...
class HybridOrchestrator:
    ...
    def execute_task(self, prompt: str) -> TaskExecutionPlan:
        ...
        for attempt in range(1, self.max_retries + 1):
            logger.info(f"Attempt {attempt}/{self.max_retries}: Requesting execution plan...")
            ...
            try:
                ...
                if validated_plan.confidence_score < 0.70:
                    logger.warning("Confidence below threshold (0.70). Flagging for human approval.")
                    validated_plan.requires_human_approval = True

                return validated_plan

            except json.JSONDecodeError as e:
                logger.error(f"Attempt {attempt} failed: Invalid JSON format from model. ({e})")
            except ValidationError as e:
                logger.error(f"Attempt {attempt} failed: Schema validation error. ({e.error_count()} errors)")

            # Backoff delay prior to retrying
            time.sleep(0.5)

        logger.critical("All probabilistic generation retries exhausted. Initiating safe fallback logic.")
